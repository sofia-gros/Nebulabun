...
use windows::{
    ...
    Win32::{
        Foundation::{COLORREF, *},
        Graphics::Gdi::{HBRUSH, UpdateWindow, CreateSolidBrush, InvalidateRect},
...
pub extern "C" fn create_webview_window(
    ...
    unsafe {
        ...
        let wc = WNDCLASSEXW {
            ...
            hbrBackground: if transparent != 0 { HBRUSH(0) } else { HBRUSH(6) },
...
pub extern "C" fn execute_javascript(script: *const c_char) -> c_int {
    unsafe {
        ...
        if let Some(ref mut instance) = WEBVIEW_INSTANCE {
            if let Some(ref webview) = instance.webview {
                let script_wide: Vec<u16> = script_str.encode_utf16().chain(std::iter::once(0)).collect();
